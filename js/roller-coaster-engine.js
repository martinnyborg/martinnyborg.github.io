$(document).ready(function() {
    ...
    var tronaldDump = ["PANIC SELLING!!!", "DUMPPP!", "Tron going to 0!! LOLOLOL", "SEGWIT POISON PILL", "LIGHTNING VAPORWARE", "buy da dip", "Tron up to 10% off", "Tronald DUMP!", "RIP McAfee's dick"];
    var hodlersBelike = ["hooodl", "hodloor", "To da mooooon", "$100K INCOMING!!!", "TRON WILL TAKE OVER THE WORLD!", "SELL WIFE! BUY TRON!!", "buckle up hodlers", "can't see any altcoin up here", "hey r/tronix, should I buy today?"];
    ...
    function fetchMooningPrices() {
        selectedMarket.runWebsocketTicker(mooningFunction);
        selectedMarket.runWebsocketTicker(updateTicker);
    }
    ...
    function mooningFunction(percentage, latest, id) {
        updateCurrentMoon(percentage, latest);
        updateStatus(percentage, latest);
        updateLabels(percentage, latest);
        updateTicker(percentage, latest, id);
        updateHodlings(latest);
    }

    function updateCurrentMoon(percentage, latest) {

        if (currentMoon != latest) {
            currentMoon = latest;
            $('#current-moon').html('$' + currentMoon + " USD");
            animateMoonElem($('#current-moon'));
            //$('#change-percentage').html(percentage + "%");
            document.title = '(' + currentMoon + ')' + " Tron Roller Coaster Guy";
            feeRequest();
        }
    }

    function updateHodlings(latest) {
        ...
        if (toggleHoldingsInput) {
            if (hodlingsAmount != undefined && hodlingsAmount != 0) {
                $('#hodler-hodlings').val((latest * hodlingsAmount).toFixed(2));
            } else {
                $('#hodler-hodlings').html('');
            }
        } else {
            if (fiatHodlingsAmount != undefined && fiatHodlingsAmount != 0) {
                $('#hodlings').val((fiatHodlingsAmount / latest).toFixed(4));
            } else {
                $('#hodlings').html('');
            }
        }
    ...
    function updateTicker(percentage, latest, id) {
        var tickerElem = $('#ticker-' + id);
        if (latest != tickerElem.html()) {
            tickerElem.html(latest);
    ...
    function updateStatus(percentage, latest) {
